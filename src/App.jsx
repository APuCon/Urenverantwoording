...
export default function App() {
  ...
  return <div className="app">
    <aside className={mobile ? "open" : ""}><div className="brand"><img className="apuBrandLogo" src="/apu-consultancy-logo.png" alt="APu Consultancy" /><div className="brandText"><b>Projecturen Manager</b><small>Projecten, uren & facturatie</small></div><Button secondary icon onClick={() => setMobile(false)}>×</Button></div><nav>{nav.map(([id, label]) => <button key={id} className={view === id ? "active" : ""} onClick={() => chooseView(id)}>{label}</button>)}</nav></aside>
